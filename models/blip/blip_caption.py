...
class BlipCaption(BlipBase):
    ...
    def sampling(
        ...
        while True:
            ...
            topk_values, topk_indices = torch.topk(next_token_logits, topk, dim=-1)  # (batch_size, topk)
            probs = torch.softmax(topk_values, dim=-1)                    # softmax over top-k
            next_tokens = torch.multinomial(probs, num_samples=1)        # sample from top-k
            next_token_ids = topk_indices.gather(-1, next_tokens)        # map sampled index back to vocab ID
