...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        image_logits = image_feat @ text_feat.t() / temperature
        text_logits = text_feat @ image_feat.t() / temperature

        batch_size = image_feat.size(0)
        targets = torch.eye(batch_size, device=device, dtype=torch.float32)
        loss_image_text = loss_fn(image_logits, targets)
        loss_text_image = loss_fn(text_logits, targets)
